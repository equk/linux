...
repos = {'Community' : 'K', 'Testing' : 'T', 'Core' : 'C', 'Extra' : 
'X', 'Community-Testing': 'Z'}
months = ('Febtober', 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 
'Aug', 'Sep', 'Oct', 'Nov', 'Dec')
hide_uninstalled = 1
 
def printLine(color, month, day, repo, pkg, pkg_ver):
print '${color %s} %s/%s %s %s %s' % (color, month, day, repo, pkg, 
pkg_ver)
...
updateCount += 1
printLine(color, months.index(date[2]), date[1], repos[repo], update[0], 
update[1])
